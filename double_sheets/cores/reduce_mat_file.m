...
clear dx_n_Sc_struct dx_n_Sc_struct_RI h_Qf_p_global_t h_Qf_p_global_t_1 h_Amat h_dt_Amat Qf_p_lift2_mat_i Qf_p_lift2_mat_i_1 dp_add_nvec_i 
%%[2] Convert to single precision
h_X_vec = single( h_X_vec);
h_X_vec(2*N_q_all+1:end,:) = [];                                                %% Remove the velocity components

h_r_wake = cellfun( @single, h_r_wake, 'UniformOutput', false);
h_r_wake_1 = cellfun( @single, h_r_wake_1, 'UniformOutput', false);
