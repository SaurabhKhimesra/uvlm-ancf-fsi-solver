...
%% [0] Shape function evaluation
[ p_vec, w_vec] = Gauss( N_gauss);          %% Gauss-Legendre quadrature points and weights
...
for ii = 1:N_element
    ...
    %%% x = 0: for the damping torque of the rotational damper at the plate leading edge
...
%% [0] Shape function evaluation
[ p_vec_RI, w_vec_RI] = Gauss( N_gauss_RI);          %% Gauss-Legendre quadrature points and weights


Sc_mat_v_RI = zeros(3,N_q,length( p_vec_RI),length( p_vec_RI),N_element);
dx_Sc_mat_v_RI = Sc_mat_v_RI;
dy_Sc_mat_v_RI = Sc_mat_v_RI;
dx2_Sc_mat_v_RI = Sc_mat_v_RI;
dy2_Sc_mat_v_RI = Sc_mat_v_RI;
dxy_Sc_mat_v_RI = Sc_mat_v_RI;
dx_Sc_mat_v_x0_RI = Sc_mat_v_RI(:,:,1,:,:);
dy_Sc_mat_v_x0_RI = Sc_mat_v_RI(:,:,1,:,:);
for ii = 1:N_element
    ...
    i_xi_a = 1;
    for xi_a = p_vec_RI                        %% Gauss-Legendre quadrature
        i_eta_a = 1;
        for eta_a = p_vec_RI
            ...
            Sc_mat_v_RI(:,:,i_xi_a,i_eta_a,ii) = Sc_mat( x_i, y_i, dL, dW);
            dx_Sc_mat_v_RI(:,:,i_xi_a,i_eta_a,ii) = dx_Sc_mat( x_i, y_i, dL, dW);
            dy_Sc_mat_v_RI(:,:,i_xi_a,i_eta_a,ii) = dy_Sc_mat( x_i, y_i, dL, dW);
            dx2_Sc_mat_v_RI(:,:,i_xi_a,i_eta_a,ii) = dx2_Sc_mat( x_i, y_i, dL, dW);
            dy2_Sc_mat_v_RI(:,:,i_xi_a,i_eta_a,ii) = dy2_Sc_mat( x_i, y_i, dL, dW);
            dxy_Sc_mat_v_RI(:,:,i_xi_a,i_eta_a,ii) = dxy_Sc_mat( x_i, y_i, dL, dW);
            
            if i_xi_a == 1
                dx_Sc_mat_v_x0_RI(:,:,1,i_eta_a,ii) = dx_Sc_mat( 0, 0*y_i, dL, dW);
                dy_Sc_mat_v_x0_RI(:,:,1,i_eta_a,ii) = dy_Sc_mat( 0, 0*y_i, dL, dW);
            end        
    ...
    dx_n_Sc_struct_RI(ii).dx_Sc_mat_v_o = dx_Sc_mat_v_RI(:,:,:,:,ii);                   
    dx_n_Sc_struct_RI(ii).dy_Sc_mat_v_o = dy_Sc_mat_v_RI(:,:,:,:,ii);
    dx_n_Sc_struct_RI(ii).dx2_Sc_mat_v_o = dx2_Sc_mat_v_RI(:,:,:,:,ii);
    dx_n_Sc_struct_RI(ii).dy2_Sc_mat_v_o = dy2_Sc_mat_v_RI(:,:,:,:,ii);
    dx_n_Sc_struct_RI(ii).dxy_Sc_mat_v_o = dxy_Sc_mat_v_RI(:,:,:,:,ii);
    %%% x = 0: for the damping torque of the rotational damper at the plate leading edge
    dx_n_Sc_struct_RI(ii).dx_Sc_mat_v_x0_o = dx_Sc_mat_v_x0_RI(:,:,:,:,ii);                   
    dx_n_Sc_struct_RI(ii).dy_Sc_mat_v_x0_o = dy_Sc_mat_v_x0_RI(:,:,:,:,ii);
    
    
    %%[*] Zero components excluded (Sc*q = [ S1*I S2*I ... S12*I]*q = S1*q1_r + S2*q1_dx_r + ... + S12*q4_dy_r, I∈R^3*3, qi_r,qi_dx_r,qi_dy_r∈R^3)
    dx_n_Sc_struct_RI(ii).dx_Sc_mat_v = repmat( dx_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1]);                   
    dx_n_Sc_struct_RI(ii).dy_Sc_mat_v = repmat( dy_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1]);
    dx_n_Sc_struct_RI(ii).dx2_Sc_mat_v = repmat( dx2_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1]);
    dx_n_Sc_struct_RI(ii).dy2_Sc_mat_v = repmat( dy2_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1]);
    dx_n_Sc_struct_RI(ii).dxy_Sc_mat_v = repmat( dxy_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1]);
    
    %%[*] Zero components excluded (Sc*q = [ S1*I S2*I ... S12*I]*q = S1*q1_r + S2*q1_dx_r + ... + S12*q4_dy_r, I∈R^3*3, qi_r,qi_dx_r,qi_dy_r∈R^3)
    %%% x = 0: for the damping torque of the rotational damper at the plate leading edge
    dx_n_Sc_struct_RI(ii).dx_Sc_mat_v_x0 = repmat( dx_Sc_mat_v_x0_RI(1,1:3:end,:,:,ii), [ 3 1 1 1]);                   
    dx_n_Sc_struct_RI(ii).dy_Sc_mat_v_x0 = repmat( dy_Sc_mat_v_x0_RI(1,1:3:end,:,:,ii), [ 3 1 1 1]);    
    
    %%[*] Zero components excluded (Sc*q = [ S1*I S2*I ... S12*I]*q = S1*q1_r + S2*q1_dx_r + ... + S12*q4_dy_r, I∈R^3*3, qi_r,qi_dx_r,qi_dy_r∈R^3)
    dx_n_Sc_struct_RI(ii).dx_Sc_mat_v2 = repmat( dx_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1 N_q]);                   
    dx_n_Sc_struct_RI(ii).dy_Sc_mat_v2 = repmat( dy_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1 N_q]);
    dx_n_Sc_struct_RI(ii).dx2_Sc_mat_v2 = repmat( dx2_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1 N_q]);
    dx_n_Sc_struct_RI(ii).dy2_Sc_mat_v2 = repmat( dy2_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1 N_q]);
    dx_n_Sc_struct_RI(ii).dxy_Sc_mat_v2 = repmat( dxy_Sc_mat_v_RI(1,1:3:end,:,:,ii), [ 3 1 1 1 N_q]);
    
    dx_n_Sc_struct_RI(ii).dx_Sc_dx_Sc_mat = mntimes( permute( dx_Sc_mat_v_RI(:,:,:,:,ii), [ 2 1 3 4]), dx_Sc_mat_v_RI(:,:,:,:,ii), 1, 2);
    dx_n_Sc_struct_RI(ii).dy_Sc_dy_Sc_mat = mntimes( permute( dy_Sc_mat_v_RI(:,:,:,:,ii), [ 2 1 3 4]), dy_Sc_mat_v_RI(:,:,:,:,ii), 1, 2);
    dx_n_Sc_struct_RI(ii).dx_Sc_dy_Sc_mat = mntimes( permute( dx_Sc_mat_v_RI(:,:,:,:,ii), [ 2 1 3 4]), dy_Sc_mat_v_RI(:,:,:,:,ii), 1, 2);
    dx_n_Sc_struct_RI(ii).dy_Sc_dx_Sc_mat = mntimes( permute( dy_Sc_mat_v_RI(:,:,:,:,ii), [ 2 1 3 4]), dx_Sc_mat_v_RI(:,:,:,:,ii), 1, 2);
    dx_n_Sc_struct_RI(ii).dxSc_dySc_p_dySc_dxSc_mat = dx_n_Sc_struct_RI(ii).dx_Sc_dy_Sc_mat + dx_n_Sc_struct_RI(ii).dy_Sc_dx_Sc_mat;
    
    
    dqdq_eps_v_RI = zeros( N_q, 3, length( p_vec_RI), length( p_vec_RI), N_q); 
    dqdq_eps_v_RI(:,1,:,:,:) = permute( dx_n_Sc_struct_RI(ii).dx_Sc_dx_Sc_mat, [ 1 3 4 2]);
    dqdq_eps_v_RI(:,2,:,:,:) = permute( dx_n_Sc_struct_RI(ii).dy_Sc_dy_Sc_mat, [ 1 3 4 2]);
    dqdq_eps_v_RI(:,3,:,:,:) = permute( dx_n_Sc_struct_RI(ii).dxSc_dySc_p_dySc_dxSc_mat, [ 1 3 4 2]);
    dx_n_Sc_struct_RI(ii).dqdq_eps_v = dqdq_eps_v_RI;
end
...
for ii = 1:N_element
    ...
    M_mat_i(:,:,ii) = int_StS;  %% Mass matrix of element ii
end



%% [2] Gravity (Qf = ∫_A S(x,y)^t*F dA, F ∈ R^3)
...
for ii = 1:N_element
    ...
    Qf_vec_i(:,ii) = int_StF;         %% External force matrix of element ii
end
