...
%% Damping coefficient (sheet 2)
theta_a = theta_a_vec(2);

%% [0] Normal strain (Qeε = t∫_A (∂q_ε)^t*Dm*ε dA ∈ R^36)
if flag_output == 1     
    
    int_e_Dp_e_vec_1 = zeros(1,N_element);

    Qe_eps_vec_i_1 = zeros(N_q,N_element);
    dq_Qe_eps_mat_i_1 = zeros(N_q,N_q,N_element);                           	%% ∫_A (dqdq_ε^T*D*ε + dq_ε^T*D*dq_ε) dA 
    Qd_eps_mat_i_1 = zeros(N_q,N_q,N_element);                                  %% ∫_A dq_ε^T*D*dq_ε dA (dq_ε*dt_q = dt_ε)    
    ...
    for ii = 1:N_element
        ...
        Qe_eps_vec_i_1(:,ii) = int_dq_e_Dp_e;                                 %% External force matrix of element ii

        int_e_Dp_e = dL*dW/4*ww_mat3_RI.*eps_Dp_eps;
        int_e_Dp_e_vec_1(ii) = sum( sum( int_e_Dp_e, 4), 3).';
        ...
        dq_Qe_eps_mat_i_1(:,:,ii) = int_dqdq_e_Dp_e_P_dq_e_Dp_dq_e;       	%% External force matrix of element ii
        ...
        if theta_a ~= 0                                                     %% Evaluated only when structural damping is present (saves computation)
            ...
            Qd_eps_mat_i_1(:,:,ii) = int_dq_e_Dp_dq_e;                        %% External force matrix of element ii
        end    
...
int_k_Dp_k_vec_1 = zeros(1,N_element);

Qe_k_vec_i_1 = zeros(N_q,N_element);
Qd_k_mat_i_1 = zeros(N_q,N_q,N_element);                                      %% ∫_A dq_κ^T*D*dq_κ dA (dq_κ*dt_q = dt_κ)
...
%%[3-0] Elastic force
Qe_global_1 = zeros(N_q_all,1);
Qk_global_1 = zeros(N_q_all,1);
for ii = 1:N_element
    ...
    if flag_output == 1 
        Qe_global_1(i_vec,1) = Qe_global_1(i_vec,1) + zeta_m*Qe_eps_vec_i_1(:,ii);
    end
    Qk_global_1(i_vec,1) = Qk_global_1(i_vec,1) + eta_m*Qe_k_vec_i_1(:,ii);
end


%%[4-1] Stiffness matrix + structural damping
dq_Qe_global_1 = sparse(N_q_all,N_q_all);
Qd_global_1 = sparse(N_q_all,N_q_all);

dq_Qe_global_vec_1 = zeros(1,length( dq_Qe_eps_mat_i(:)));
Qd_global_vec_1 = zeros(1,length( Qd_eps_mat_i(:)));
if flag_output == 1 
    ...
    for ii = 1:N_element
        ...
        dq_Qe_global_vec_i_1 = zeta_m*dq_Qe_eps_mat_i_1(:,:,ii);
        dq_Qe_global_vec_1((ii - 1)*N_q^2+(1:N_q^2)) = dq_Qe_global_vec_i_1(:).';

        if theta_a ~= 0                                                                 %% Evaluated only when structural damping is present (saves computation)
            
            Qd_global_vec_i_1 = theta_a*zeta_m*Qd_eps_mat_i_1(:,:,ii) +  theta_a*eta_m*Qd_k_mat_i_1(:,:,ii);
            Qd_global_vec_1((ii - 1)*N_q^2+(1:N_q^2)) = Qd_global_vec_i_1(:).';
        end     
    end

    dq_Qe_global_1 = setsparse( dq_Qe_global_1, I_vec, J_vec, dq_Qe_global_vec_1, @plus);
    if theta_a ~= 0                                                                     %% Evaluated only when structural damping is present (saves computation)
        Qd_global_1 = setsparse( Qd_global_1, I_vec, J_vec, Qd_global_vec_1, @plus);
    end
