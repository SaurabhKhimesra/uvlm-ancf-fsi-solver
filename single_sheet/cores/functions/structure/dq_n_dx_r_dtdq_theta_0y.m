...
dq_n_norm_n_dx_r_norm_dx_r_dtdq_theta_0y_v = dq_n_norm_n_dx_r_norm_dx_r_dtdq_theta_0y( q_i_vec, dt_q_i_vec, n_vec_norm_n, dx_n_Sc_struct, N_q, norm_n);

out = dq_n_norm_n_dx_r_norm_dx_r_dtdq_theta_0y_v;
...
dtdx_r = mntimes2_fast( dx_Sc_mat_v, dt_q_vec);
...
dtdx_rT_ex_ezT_m_ez_exT = sum( dtdx_r(:,ones(1,3),:,:).*ex_ezT_m_ez_exT(:,:,ones(1,size( dtdx_r, 3)),ones(1,size( dtdx_r, 4))), 1);
dtdx_rT_ex_ezT_m_ez_exT_dx_Sc_mat = repmat( permute( dtdx_rT_ex_ezT_m_ez_exT, [ 2 1 3 4]), [ 1 N_q/3 1 1]).*dx_Sc_mat_v;
dtdx_rT_ex_ezT_m_ez_exT_dx_Sc_mat = reshape( dtdx_rT_ex_ezT_m_ez_exT_dx_Sc_mat, 1, [], size( dtdx_r, 3), size( dtdx_r, 4));

dxr_ex_2_p_dxr_ez_2 = repmat( dx_r(1,:,:,:).^2 + dx_r(3,:,:,:).^2, [ 1 N_q 1 1]);


ex_exT_p_ez_ezT = ex*ex.' + ez*ez.';
dx_rT_ex_exT_p_ez_ezT = sum( dx_r(:,ones(1,3),:,:).*ex_exT_p_ez_ezT(:,:,ones(1,size( dx_r, 3)),ones(1,size( dx_r, 4))), 1);
dx_rT_ex_exT_p_ez_ezT_dtdx_r = sum( permute( dx_rT_ex_exT_p_ez_ezT, [ 2 1 3 4]).*dtdx_r, 1);
...
dx_rT_ex_exT_p_ez_ezT_dtdx_r_dx_rT_ex_ezT_m_ez_exT_dx_Sc_mat = dx_rT_ex_exT_p_ez_ezT_dtdx_r(1,ones(1,N_q),:,:).*dx_rT_ex_ezT_m_ez_exT_dx_Sc_mat;

dtdq_theta_0y = 2*dx_rT_ex_exT_p_ez_ezT_dtdx_r_dx_rT_ex_ezT_m_ez_exT_dx_Sc_mat./dxr_ex_2_p_dxr_ez_2.^2 ...
                                - dtdx_rT_ex_ezT_m_ez_exT_dx_Sc_mat./dxr_ex_2_p_dxr_ez_2;


out = -dq_n_per_norm_nT_dx_r_mat_per_norm_dx_r(:,ones(1,N_q),:,:).*dtdq_theta_0y(ones(1,N_q),:,:,:);
