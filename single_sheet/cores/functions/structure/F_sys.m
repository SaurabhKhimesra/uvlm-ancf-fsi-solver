function f_sys = F_sys( X_vec, M_global, Qf_global, Qe_global, Qd_global, var_param)
...
D_matrix = [ eye_mat                zero_mat;
             M_global\Qd_global     eye_mat];                                                        	%% Reduces to the identity matrix at zero damping.
...
%% [3] Right-hand side of the state equation
% f_sys = [   dt_q_vec;
%             dtt_q_vec];

f_sys_0 = D_matrix\[  dt_q_vec(n_i_vec);
                      M_global\Q_global];
                    
f_sys =  zeros(2*N_q_all,1);
f_sys([ n_i_vec N_q_all+n_i_vec]) = f_sys_0;
