...
dp_sum_vec = h_dp_vec(:,1:length( time_wake_m(1:end-1)));                    	%% シート法線方向流体圧力 [-]

%%[*] スパン中央の圧力分布を出して，揚力を計算する．
%%%
...
if mod( Ny, 2) == 0
    ...
    CL_vec = 2*Ny/A_total*(   dA_vec(idx_cord)*dp_sum_vec(idx_cord,:) + ...
                    dA_vec(idx_cord+1)*dp_sum_vec(idx_cord+1,:) )/2.*cos( theta_pitch_time_m);                  %% 流れ鉛直方向流体力（揚力成分）[-]
else
    
    idx_cord = floor( Ny/2):Ny:N_element;   
    
    CL_vec = 2*Ny/A_total*(  dA_vec(idx_cord+1)*dp_sum_vec(idx_cord+1,:) ).*cos( theta_pitch_time_m);           %% 流れ鉛直方向流体力（揚力成分）[-]
end
