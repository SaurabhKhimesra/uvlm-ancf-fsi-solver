...
%% Snapshot of the mid-span displacement
...
locs( pks <= 0) = [];%% Noise removal

if length( locs) >= 3
    plot_velocity_flag = 1;
else
    plot_velocity_flag = 0;
end


if plot_velocity_flag
    idx_time_Tp = locs(end-2):locs(end-1);

    %%[*-0] 0, 1/4, 1/2 period
    idx_time_Tp_vec = idx_time_Tp(round( [ 1 end/4 end/2]));
else
    idx_time_Tp = 1:round( i_time*3/4);
    
    idx_time_Tp_vec = [ 1 round( i_time/2) round( i_time*3/4)];
end
...
idx_time_Tp_snapshot = round( linspace( idx_time_Tp(1), idx_time_Tp(end), 50));


plot( h_ax(i_ax), X_center_disp(:,idx_time_Tp_snapshot), Z_center_disp(:,idx_time_Tp_snapshot), 'b-', 'LineWidth', 0.2)

set( h_ax(i_ax), 'FontName', 'Times New Roman', 'FontSize', 12)
axis( h_ax(i_ax), 'equal')
xlabel( h_ax(i_ax), '{\itX}^* position', 'FontName', 'Times New Roman', 'FontSize', 15)
ylabel( h_ax(i_ax), '{\itZ}^* position', 'FontName', 'Times New Roman', 'FontSize', 15)
xlim( h_ax(i_ax), [ 0 1])
...
%% Time history of the mid-span free-end displacement
...
%% Phase plane of the mid-span free-end displacement and velocity
...
set( h_ax(i_ax), 'FontName', 'Times New Roman', 'FontSize', 12)
xlabel( h_ax(i_ax), '{\itZ}^* position', 'FontName', 'Times New Roman', 'FontSize', 15)
ylabel( h_ax(i_ax), '{\itZ}^* velocity', 'FontName', 'Times New Roman', 'FontSize', 15)
...
%% Fluid model bench (Theodorsen bench)


%%[*] UVLM
theta_pitch_time_m = theta_pitch_time( time_wake_m(1:end-1));
dt_theta_pitch_time_m = dt_theta_pitch_time( time_wake_m(1:end-1));
dtt_theta_pitch_time_m = dtt_theta_pitch_time( time_wake_m(1:end-1));

A_total = Length*Width;

dA_vec = dL_vec.*dW_vec;
dp_sum_vec = h_dp_vec(:,1:length( time_wake_m(1:end-1)));                    	%% Fluid pressure normal to the sheet [-]

%%[*] Take the mid-span pressure distribution and compute the lift.
%%%
%%% CL := L/(1/2ρU^2A)  = [P]A/(1/2ρU^2A) 
%%%     = [P]/(1/2ρU^2) = 2[P]/(ρU^2) =: 2[p] 
%%%
if mod( Ny, 2) == 0
    ...
    CL_vec = 2*Ny/A_total*(   dA_vec(idx_cord)*dp_sum_vec(idx_cord,:) + ...
                    dA_vec(idx_cord+1)*dp_sum_vec(idx_cord+1,:) )/2.*cos( theta_pitch_time_m);                  %% Fluid force normal to the flow (lift component) [-]
else
    
    idx_cord = floor( Ny/2):Ny:N_element;   
    
    CL_vec = 2*Ny/A_total*(  dA_vec(idx_cord+1)*dp_sum_vec(idx_cord+1,:) ).*cos( theta_pitch_time_m);           %% Fluid force normal to the flow (lift component) [-]
end

%%[*] Theodorsen Solution
theta_pitch_theory = Theta_pitch*exp( 1i*omega_pitch*time_wake_m(1:end-1));
% CL_theory = 2*pi*( real( C_theodorsen( 1+1i*k_omega)) + imag( C_theodorsen( 1i - k_omega)))*theta_pitch_theory + pi*(1i - k_omega/2)*theta_pitch_theory;
CL_theory = pi*(2 + 1i*k_omega)*C_theodorsen( k_omega)*theta_pitch_theory + 1i*pi*k_omega*theta_pitch_theory;


h_fig(9) = figure(9);
set( h_fig(9), 'Position', [100 100 600 500])

h_ax(i_ax) = axes( 'Parent', h_fig(9));

h_plot_CL(1) = plot( h_ax(i_ax), theta_pitch_time_m(end/2:end), CL_vec(end/2:end), 'b--', 'LineWidth', 2, 'DisplayName', 'UVLM');
hold( h_ax(i_ax), 'on')
h_plot_CL(2) = plot( h_ax(i_ax), theta_pitch_theory, CL_theory, 'r-', 'LineWidth', 1, 'DisplayName', 'Theodorsen');

h_leg = legend( h_plot_CL);
set( h_leg, 'FontName', 'Times New Roman', 'FontSize', 15, 'Location', 'NorthWest')

set( h_ax(i_ax), 'FontName', 'Times New Roman', 'FontSize', 12)
xlabel( h_ax(i_ax), '{\it\alpha}({\itt}) [rad]', 'FontName', 'Times New Roman', 'FontSize', 25)
ylabel( h_ax(i_ax), '{\itC_L}({\itt})', 'FontName', 'Times New Roman', 'FontSize', 25)
xlim( h_ax(i_ax), [ 1.5*min( theta_pitch_time_m) 1.5*max( theta_pitch_time_m)])
% ylim( h_ax(i_ax), [ -1.5 1.5])
...
for i_time = idx_time_Tp_vec(1)
    ...
    %%[*] Map the structural time onto the fluid time index.
    time = time_m(i_time);
    
    [ dummy, i_wake_time] = min( abs( time_wake_m - time));
    ...
    %%[*] plot
    %%[*] U velocity
    h_fig(i_fig) = figure(i_fig);
    set( h_fig(i_fig), 'Position', [100 100 1200 500])
    h_ax(i_ax) = axes( 'Parent', h_fig(i_fig));
    ...
    grid( h_ax(i_ax), 'on')
    hold( h_ax(i_ax), 'on')
    set( h_ax(i_ax), 'FontName', 'Times New Roman', 'FontSize', 15, 'LineWidth', 2)

    h_col = colorbar;
    ylabel( h_col, '{\itu}^*', 'FontName', 'Times New Roman', 'FontSize', 25)
    caxis( h_ax(i_ax), [ 0 2])

    xlim( h_ax(i_ax), [-Length 5*Length])
    ylim( h_ax(i_ax), [-1.5*Length 1.5*Length])
    xlabel( h_ax(i_ax), '{\itX}^* position', 'FontName', 'Times New Roman', 'FontSize', 25)
    ylabel( h_ax(i_ax), '{\itZ}^* position', 'FontName', 'Times New Roman', 'FontSize', 25)

    plot( h_ax(i_ax), X_center_disp(:,i_time), Z_center_disp(:,i_time), 'b-', 'LineWidth', 4)
    colormap jet
    ...
    h_col = colorbar;
    ylabel( h_col, '{\itw}^*', 'FontName', 'Times New Roman', 'FontSize', 25)
    caxis( h_ax(i_ax), [ -2 2])
    ...
    %%[*] u_norm velocity (u := U/U_in)
    h_fig(i_fig) = figure(i_fig);
    set( h_fig(i_fig), 'Position', [100 100 1200 500])
    h_ax(i_ax) = axes( 'Parent', h_fig(i_fig));

    contourf( h_ax(i_ax), rx_mat, rz_mat, u_norm_mat, 40, '-.')
    view( h_ax(i_ax), [0 0 1])
    axis( h_ax(i_ax), 'equal')
    grid( h_ax(i_ax), 'on')
    hold( h_ax(i_ax), 'on')
    ...
    h_col = colorbar;
    ylabel( h_col, '|{\bf \itu}^*|', 'FontName', 'Times New Roman', 'FontSize', 25)
    caxis( h_ax(i_ax), [ 0 2])
